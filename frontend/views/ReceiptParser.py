        ...
        if st.session_state["pending_receipts"]:
            ...
            if st.button("💾 Save All Pending Transactions"):
                with st.spinner("Saving transactions..."):
                    ...
                    st.session_state["uploaded_files"] = []
                    st.session_state["processing_status"] = {}
                    st.session_state["just_saved"] = True  # Mark that we just saved
                    st.success("✅ Added transactions successfully!")
                    st.rerun()
    ...
    if "transactions" in st.session_state:
        ...
        if receipt_tx:
            if st.session_state["just_saved"]:
                ...
                # Add a button to start new receipt upload
                if st.button("📸 Scan New Receipts"):
                    st.session_state["just_saved"] = False
                    st.rerun()
                st.markdown("---")
